...
LATEST_TAG=$(curl -s https://registry.hub.docker.com/v2/repositories/orebenson/smart-towns/tags/?page_size=100 | jq -r '.results | map(.name) | sort_by(split("_")[0] | tonumber) | .[-1]')
echo "PULLING IMAGE VERSION $LATEST_TAG with format: [jenkins build no.]_[git commit hash]"
sudo docker pull orebenson/smart-towns:$LATEST_TAG
sudo docker run -e SERVER_PORT=8081 -p 8081:8081 --name myapp1 -d --network host orebenson/smart-towns:$LATEST_TAG
sudo docker run -e SERVER_PORT=8082 -p 8082:8082 --name myapp2 -d --network host orebenson/smart-towns:$LATEST_TAG
sudo docker run -e SERVER_PORT=8083 -p 8083:8083 --name myapp3 -d --network host orebenson/smart-towns:$LATEST_TAG
